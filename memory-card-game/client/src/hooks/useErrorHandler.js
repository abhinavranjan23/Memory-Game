...
const useErrorHandler = () => {
      ...
      if (error.response) {
        ...
        switch (status) {
          ...
          case 404:
            ...
            break;
          case 409:
            message = data.message || 'Conflict: This action cannot be completed.';
            break;
          case 429:
  ...
  const handleGameError = useCallback((error) => {
    const gameSpecificMessages = {
      ...
      'GAME_NOT_FOUND': 'Game room not found. It may have ended or been removed.',
      ...
      'UNAUTHORIZED_ACTION': 'You\'re not authorized to perform this action in this game.'
    };

    const message = gameSpecificMessages[error.code] || error.message || 'A game error occurred.';
    const type = ['CONNECTION_LOST', 'PLAYER_DISCONNECTED'].includes(error.code) ? 'warning' : 'error';
