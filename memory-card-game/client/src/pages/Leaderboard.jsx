...
const Leaderboard = () => {
  ...
  const fetchLeaderboard = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/game/leaderboard/global', {
        params: {
          limit: 50,
          page,
  ...
  const fetchGlobalStats = async () => {
    try {
      const response = await axios.get('/game/stats/global');
          ...
          <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{globalStats.totalPlayers?.toLocaleString()}</p>
              </div>
            </div>
          </div>
          <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{globalStats.totalGames?.toLocaleString()}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {Math.round((globalStats.averageGameDuration || 0) / 60000)}m
                </p>
              </div>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {activeTab === 'totalScore' ? 'Total Score' :
                     activeTab === 'winRate' ? 'Win Rate' : 'Games Played'}
                  </th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    Games
                  </th>
                  ...
                  {filteredLeaderboard.map((player, index) => (
                    <motion.tr
                      ...
                      className={`
                        ...
                        ${isCurrentUser(player.id) ? 'bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500' : ''}
                      `}
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-bold text-gray-900 dark:text-white">
                          {activeTab === 'totalScore' ? player.totalScore?.toLocaleString() :
                           activeTab === 'winRate' ? `${player.winRate}%` :
